...
export const register =
	...
	async (dispatch) => {
		...
		try {
			const res = await axios.post(`${baseApi}/api/users`, body, config);
			dispatch({
...
export const loadUser = () => async (dispatch) => {
	...
	try {
		const res = await axios.get(`${baseApi}/api/users/auth`);
		dispatch({
			type: USER_LOADED,
			payload: res.data,
		});
...
export const login = (email, password) => async (dispatch) => {
	...
	try {
		const res = await axios.post(`${baseApi}/api/users/login`, body, config);
		dispatch({
			type: LOGIN_SUCCESS,
			payload: res.data,
		});
...
export const logout = () => async (dispatch) => {
	...
	const body = '';
	try {
		const res = await axios.post(`${baseApi}/api/users/logout`, body, config);
		if (res.status === 200) {
	...
	if (
		...
		try {
			const res = await axios.delete(
				`${baseApi}/api/users/deleteAccount`,
				body,
				config
			);
			if (res.status === 200) {
